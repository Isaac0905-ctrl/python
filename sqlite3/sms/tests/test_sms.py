...
def test_build_sms_works_with_special_phone_numbers():
    sms = SMS(sender='+34 675118923', recipient='+51 721895643', message='Hi there. Are you ok?')
    assert sms.sender == '+34 675118923'
    assert sms.recipient == '+51 721895643'


@pytest.mark.parametrize(
    'sender', ('67511892', '6751189234', 'abcde1234', '67-5118923', '+34 6751189234')
)
def test_build_sms_fails_with_invalid_sender_phone_format(sender: str):
    with pytest.raises(SMSError) as err:
        SMS(sender=sender, recipient='721895643', message='Hi there. Are you ok?')
    assert str(err.value) == 'Sender has invalid phone format'


@pytest.mark.parametrize(
    'recipient', ('72189564', '7218956434', 'abcde1234', '72-1895643', '+34 6751189234')
)
def test_build_sms_fails_with_invalid_recipient_phone_format(recipient: str):
    with pytest.raises(SMSError) as err:
        SMS(sender='675118923', recipient=recipient, message='Hi there. Are you ok?')
...
def test_sim_unlock_works_with_right_pin(sim1: SIM):
    sim1.unlock('1167')
...
def test_sim_unlock_fails_when_pin_and_puk_are_wrong(sim1: SIM):
    with pytest.raises(SMSError) as err:
        sim1.unlock('1000', puk='000000')
    assert str(err.value) == 'SIM cannot be unlocked'
